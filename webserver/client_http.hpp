...
namespace SimpleWeb {
  ...
  protected:
    ...
    class Session {
    public:
      Session(std::shared_ptr<Connection> connection, std::unique_ptr<asio::streambuf> request_buffer) noexcept
...
shared_ptr<int[]> b(new int[4]{1,2,3,4});
shared_ptr<int[]> c = make_shared<int[]>();

template <typename T> class fnc
{
