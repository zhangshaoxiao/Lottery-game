...
namespace SimpleWeb {
  ...
  public:
    ...
    public:
	  ...
	  //如果为真，则在发送响应后强制服务器关闭连接。 这在实现HTTP / 1.0服务器发送内容时非常有用
	  //而不指定内容长度。
  ...
  protected:
    ...
    public:
      ...
      void close() noexcept {
        error_code ec;
        std::unique_lock<std::mutex> lock(socket_close_mutex); 
